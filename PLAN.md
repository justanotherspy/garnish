...
- [ ] Left open by the 2026-09-25 review, each small:
  ...
  - config location, Daniel's call (verification of 2026-09-26): a config
    ...
    target. A settings file serde refuses but Claude Code applies (a lone
    `\ud800` in a string) is skipped by the hand commands, so a lower
    file's `GARNISH_CONFIG` or command names the config where the ticks
    read the refused file's (never a checkout's choice, since a skipped
    file chooses nothing; refusing the whole chain instead would stop
    every command in such a checkout). The drop-in order (the last by
    name wins, every drop-in above the managed file) is read from the
    settings documentation, not the binary; a relative `export
