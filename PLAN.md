...
- [x] ~~Lock horizon (SPEC § 6)~~ dropped in the spec review of 2026-09-12: `LOCK_STALE_MS` already bounds a lock's life at 60 s / 15 s whatever its pid, so a 24 h horizon could never fire (SPEC § 6 notes it)
...
- **2026-09-12 (FUTURE-SPEC review, documents only)** — Daniel asked for
  ...
  model inside out. An adversarial review of the day's spec text (a
  subagent briefed to find contradictions and gaps against the rest of
  SPEC and CLAUDE.md) returned 25 findings, all taken: the two samples
  contradicted the box rules (an unstyled box under `style = "none"` is
  now `rounded`, a box's `fill` defaults to `false`, and samples are
  drawn at their box width, not a column count); the 24 h lock horizon
  could never fire (`LOCK_STALE_MS` already bounds a lock's life) and was
  dropped; `path.depth` already existed; and a dozen undefined corners
  were defined: title placement inside boxes, on multi-row lines and
  without a rule, `align` counting per justified end and only between
  lines with the same column count, `auto` columns jittering with their
  content and never scrolling, clamping when the width runs out, an
  emptied column keeping its share, `blank` on the outer line, box rows
  replacing the frame's caps, the animate precedence (env, config,
  settings, default), `reset` widths under `fixed`, `usable` fallbacks,
  percent-encoded branch links, `max_width` rejected on text modules,
  the placement map's ranges for cut and hidden modules, frozen clock and
  `TZ` for setup snapshots, the tty rules per command, one backup rule
  shared with `install`, the terminal-restore hook ahead of color-eyre's,
  `IconSpec` as the real struct name, and caps on columns, inner lines
  and `title_pad`. The row prefix of Phase 19 means that with colour on no
  configured row is ever dropped by the harness, so `blank` matters only
  with colour off; § 2.1 and § 4.1 say so.
  ...
  rule for unparsable files, the 24 h lock horizon (dropped later that
  day, below), `max_width` (A5),
