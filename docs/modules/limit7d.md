...
| `reset` | `countdown` \| `absolute` \| `both` | `"countdown"` | `"countdown"` | `"countdown"` | How the reset shows: `countdown` (`⏱3d4h`), `absolute` the local wall-clock time with its weekday, since the reset is days away (`⏱Tue 14:30`), or `both` (`3d4h (Tue 14:30)`); `show_reset = false` hides every form. |
