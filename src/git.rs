...
///   `git status` run inside it. `core.trustctime` is left to the
///   repository: pinned to `true`, it made a tree git calls clean dirty for
///   good where it is `false`, and without ctime a crafted index still has
///   to match the file's inode.
///
...
mod tests {
    ...
    /// The dirty check keeps the repository's own `core.trustctime`: git's
    /// docs advise `false` where a backup tool, a crawler or an attribute
    /// writer touches ctime, and no refresh the user runs rewrites the
    /// index's ctime, so pinning it to `true` (the final review of
    /// 2026-09-25 tried) showed such a clean tree as dirty for good.
    #[test]
    fn a_ctime_the_repository_does_not_trust_leaves_the_tree_clean() {
        let (_d, work) = repo();
        let t = Duration::from_secs(5);
        git(&work, &["config", "core.trustctime", "false"]);
        git(&work, &["update-index", "--refresh"]);
        ...
        let status = Command::new("git")
            ...
            .output()
            .unwrap();
        assert!(status.stdout.is_empty(), "git itself calls the tree clean");
        assert_eq!(is_dirty(&work, t), Ok(false));
    }
